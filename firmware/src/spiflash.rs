...
impl<SPI, CS: OutputPin> SPIFlash<SPI, CS> {
    pub fn new(spi: SPI, mut cs: CS) -> Self {
        ...
        modif!(DMA1.st[4].m0ar: m0a = DMABUF.as_mut_ptr() as u32);
        modif!(DMA1.st[4].par: pa = (&*pac::SPI2::ptr()).dr().as_ptr() as u32);
        // Stream 3: RX
        modif!(DMA1.st[3].cr: msize = 0, psize = 0,
               minc = true, pinc = false, dbm = false, ct = false, circ = false,
               dir = @peripheral_to_memory, chsel = 0);
        modif!(DMA1.st[3].m0ar: m0a = DMABUF.as_mut_ptr() as u32);
        modif!(DMA1.st[3].par: pa = (&*pac::SPI2::ptr()).dr().as_ptr() as u32);
