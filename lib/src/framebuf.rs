...
pub const BLACK_ON_WHITE: &Palette = &[15, 7, 8, 0];
pub const RED_ON_WHITE: &Palette = &[15, 217, 203, 160];
pub const GREEN_ON_WHITE: &Palette = &[15, 156, 82, 34];

/// A single font, with 256 characters (cp437 encoding for text fonts).
///
/// Character data is stored in a big array, with the start of each
/// character stored separately, so that characters can overlap,
/// or be completely reused between different code points.
pub struct Font {
    /// Bitmap data, in 2bpp.
    data:  &'static [u8],
...
/// Framebuffer fonts, files are generated by `util/regen-fonts`.
...
/// Abstraction for different implementations of framebuffers, which can use
/// e.g. hardware acceleration for certain operations.
pub trait FbImpl {
...
/// Represents a single 8-bit framebuffer.
...
impl<'buf, Fb: FbImpl> FrameBuffer<'buf, Fb> {
    ...
    pub fn text(&mut self, font: &Font, mut px: u16, py: u16, text: &[u8], pal: &Palette) {
        let size = font.size();
        for &chr in text {
            self.image(px, py, font.data(chr), size, pal);
            px += size.0;
            if px >= self.width {
                return;
            }
    ...
    pub fn image(&mut self, px: u16, py: u16, img: &[u8], size: (u16, u16), pal: &Palette) {
        let mut bits = 0x1;
        let mut off = 0;
        for y in 0..size.1 {
            for x in 0..size.0 {
                ...
                self.set_pixel(px + x, py + y, pal[(bits & 0b11) as usize]);
                bits >>= 2;
            }
        }
    }
